...
class Find_Words: # 根据pmi选择 ngram
...
def request_features_from_stanford(data_dir, flag):
    all_sentences, _ = read_txt(path.join(data_dir, flag + '.txt')) # 二位列表
    sentences_str = []
    for sentence in all_sentences:
        sentence = [change(i) for i in sentence]  # 替换掉小括号
        # if sentence[-1] == '·':
        #     sentence[-1] = '.'
